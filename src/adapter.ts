...
export class MongoDbAdapter<T, D = Partial<T>, P extends MongoDBAdapterParams = MongoDBAdapterParams> extends AdapterBase<MongoDBAdapterOptions> {
  ...
  _multiOptions (id: NullableId|ObjectId, params: MongoDBAdapterParams = {}) {
    ...
    const options = Object.assign({ multi: true }, (params.mongodb != null) || params.options)
  ...
  _options (params: MongoDBAdapterParams = {}) {
    ...
    const options = Object.assign({}, (params.mongodb != null) || params.options)
  ...
  async _find (params: P = {} as P): Promise<Paginated<T>|T[]> {
    ...
    let runQuery = async (total: number) => {
      return await q.toArray().then(data => {
        return {
          total,
          limit: filters.$limit,
          skip: filters.$skip || 0,
  ...
  async _create (data: Array<Partial<D>>, params?: P): Promise<T[]>;
  async _create (data: Partial<D>|Array<Partial<D>>, _params?: P): Promise<T|T[]>;
  async _create (data: Partial<D>|Array<Partial<D>>, params: P = {} as P): Promise<T|T[]> {
