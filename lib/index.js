...
class Service extends AdapterService {
  ...
  _objectifyIdQuery (query) {
    Object.keys(query).forEach((key) => {
      if (['$or', '$and'].includes(key)) {
        query[key].forEach((_, subKey) => {
          this._objectifyIdQuery(query[key][subKey]);
        })
      }

      if (key === this.id && typeof query[this.id] === 'object') {
        Object.keys(query[this.id]).forEach((subKey) => {
          query[this.id][subKey] = Array.isArray(query[this.id][subKey])
            ? query[this.id][subKey].map((v) => this._objectifyId(v))
            : this._objectifyId(query[this.id]);
        });
      } else if (key === this.id) {
